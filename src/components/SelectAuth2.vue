        ...
        <div class="box-body">
          <ul class="left-part">
            <li @click="switchMenu($event, item, index)" :class="{active: changeFlag === index}"
                v-for="(item, index) in fatherMenu"><a href="javascript:void(0)">请选择{{item.labelName}}</a>
            </li>
          </ul>
            ...
            <div class="data-list">
              <div v-show="changeFlag === index" v-for="(item, index) in authSelect">
                <el-checkbox :indeterminate="item.isIndeterminate"
                             v-model="item.isAll"
                             @change="handleCheckAllChange($event, subOpt, item)">全选/全不选
                </el-checkbox>
                <div class="select-auth-input">
                  <el-input placeholder="请输入关键词" icon="search" v-model="input" @change="refreshHub"></el-input>
                </div>

                <div v-for="(val, index) in subOpt" v-show="searchResult(val)" >
                  <!--原声写法-->
                  <!--<input type="checkbox" v-model="item.checked"  :value="val" @click="handleCheckedItemsChange(subOpt, item, $event)">-->
                  <!--<span>{{val}}</span>-->

                  <div class="cell" >
                    <label class="el-checkbox" @click="handleCheckedItemsChange(subOpt, item, $event)">
...
//        console.dir(_filterData)
      ...
      // 半选状态控制
      handleCheckedItemsChange (subOpt, authItem, event) {
        // 判断是否全选
        let _checkedCount = authItem['checked'].length
        authItem['isAll'] = _checkedCount === subOpt.length
        // 判断是否打开半选
        authItem.isIndeterminate = authItem['checked']['length'] > 0 && authItem['checked']['length'] < subOpt.length
        // 清空输入框，便于直接输入
        this.input = ''
      },
      // 展示搜索结果
